...
double Spline::Basis(const breakpoints &v,int k, int j, double x, int der) const{
    if(der <=0 ){
        if (k <= 0)
        {
            ...
            if(  ((x>v[j+1]) || (x < v[j-k] ) ) &&  ((x>=v[v.p()-1])&&(j<v.p()-2) ) && ( (x<v[1])&&(j>k) ) )
                return 0.;
...
public:
    ...
    void fit1d(ArrayRef<double> jX, ArrayRef<double> jY, ArrayRef<long> k, bool sorted);

    void fitNd(ArrayRef<double,2> jX, int nX2, ArrayRef<double> jY, ArrayRef<long> k, bool sorted);
...
void SplineJulia::fit1d(ArrayRef<double> jX, ArrayRef<double> jY, ArrayRef<long> jk, bool sorted)
{
    int nX = jX.size();
    int nY = jY.size();
    Map<MatrixXd> X(&jX[0],nX,1);
    Map<VectorXd> Y(&jY[0],nY);
...
void SplineJulia::fitNd(ArrayRef<double,2> jX, int nX, ArrayRef<double> jY, ArrayRef<long> jk, bool sorted)
{
    int nY = jY.size();
    Map<MatrixXd> X(&jX[0],nY,nX);
    Map<VectorXd> Y(&jY[0],nY);
...
double SplineJulia::eval(ArrayRef<double> jx)
{
    int nx = jx.size();
...
double SplineJulia::eval1d(double x)
{
    Matrix<double,1,1> X;
    X(0) = x;
    return f(X);
...
JULIA_CPP_MODULE_BEGIN(registry)
    Module& spline = registry.create_module("NDSpline");
    spline.add_type<SplineJulia>("Spline")
.constructor<>()
.method("fit",&SplineJulia::fit1d)
.method("fit",&SplineJulia::fitNd)
